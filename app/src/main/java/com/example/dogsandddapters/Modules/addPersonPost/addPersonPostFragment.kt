...
class AddPersonPostFragment : Fragment() {
    ...
    private fun setupUI(view: View) {
        ...
        btnPost.setOnClickListener {
            ...
            if (request.isBlank() || offer.isBlank() || contact.isBlank()) {
                Toast.makeText(requireContext(), "Please fill in all fields", Toast.LENGTH_SHORT).show()
            } else {
                val currentUser = FirebaseAuth.getInstance().currentUser
                currentUser?.let { user ->
                    val postID: String = UUID.randomUUID().toString()
                    val publisher = user.uid

                    val bitmap = (imageView.drawable as? BitmapDrawable)?.bitmap

                    bitmap?.let { imageBitmap ->
                        uploadImageToFirebaseStorage(imageBitmap) { imageUrl ->
                            val personPost = PersonPost(postID, publisher, request, offer, contact, imageUrl)
                            PersonPostModel.instance.addPersonPost(personPost) {
                                navigateToGeneralPostsFragment(view)
                            }
                        }
                    } ?: run {
                        val personPost = PersonPost(postID, publisher, request, offer, contact, "")
                        PersonPostModel.instance.addPersonPost(personPost) {
                            navigateToGeneralPostsFragment(view)
                        }
                    }
    ...
    private fun navigateToGeneralPostsFragment(view: View) {
        val action = addPersonPostFragmentDirections.actionAddPersonPostFragmentToGeneralPostsFragment()
        Navigation.findNavController(view).navigate(action)
    }
